...
public class FFmpegMediaMetadataRetriever {
    ...
    public FFmpegMediaMetadataRetriever(IjkLibLoader libLoader) {
        initMediaMetadataRetriever(libLoader);
    }

    private void initMediaMetadataRetriever(IjkLibLoader libLoader) {
        ...
        //maybe need save object weak ref to native
        native_setup();
    }
    ...
    public Bitmap getFrameAtTime(long timeUs, int option) {
        ...
        int width = data.getInt(METADATA_KEY_VIDEO_WIDTH);
        int height = data.getInt(METADATA_KEY_VIDEO_HEIGHT);
        ...
        int ret = _getFrameAtTime(timeUs, option, b);
        return ret < 0 ? null : b;
    }
    ...
    public Bitmap getScaledFrameAtTime(long timeUs, int option, int width, int height) {
        ...
        int ret = _getFrameAtTime(timeUs, OPTION_CLOSEST_SYNC, bitmap);

        return ret < 0 ? null : bitmap;
    }
    ...
    private native int _getFrameAtTime(long timeUs, int option, Bitmap bitmap);
    ...
    public Bitmap getEmbeddedPicture() {
        ...
        Bitmap.Config config = Bitmap.Config.RGB_565;
        Bitmap bitmap = Bitmap.createBitmap(width, height, config);
        int ret = _getEmbeddedPicture(bitmap);
